...
const PokemonCompare = () => {
  ...
  useEffect(() => {
    axios
      ...
      .catch((error) =>
        console.error("Error fetching first Pokémon list:", error)
      );
    ...
    axios
      ...
      .catch((error) =>
        console.error("Error fetching second Pokémon list:", error)
      );
  }, []);
  ...
  // handle comparison when click on compare button
  const handleCompareClick = () => {
    setShowComparison(true);
  };

  const fetchMovesData = async (moves, setPokemonData) => {
    const movesPromises = moves
      .slice(0, 5)
      .map((move) => axios.get(move.move.url));
    try {
          ...
          <div className="comparison-details fade-in">
            ...
            {pokemonData2 && (
              <div className="pokemon-details pokemon-details2">
                ...
                <img
                  src={pokemonData2.sprites.front_default}
                  alt={pokemonData2.name}
                />
                <p>
                  <span className="card-title">Types:</span>{" "}
                  {pokemonData2.types.map((type) => type.type.name).join(", ")}
                </p>
                <p>
                  <span className="card-title">Abilities:</span>{" "}
                  {pokemonData2.abilities
                    .map((ability) => ability.ability.name)
                    .join(", ")}
                </p>
                <p>
                  <span className="card-title">Height:</span>{" "}
                  {pokemonData2.height} | Weight: {pokemonData2.weight}
                </p>
                <p>
                  <span className="card-title">Base Experience:</span>{" "}
