...
const Bookmarks = () => {
  const { updatedPokeList, bookmarkStatus, setBookmarkStatus } =
  ...
  const getBookmarkedPokemonDetails = () => {
    const bookmarkedIds = Object.keys(bookmarkStatus).filter(
      (id) => bookmarkStatus[id]
    );
    console.log("id", bookmarkedIds.join(","));
    console.log(updatedPokeList);

    const bookmarkedPokemonDetails = updatedPokeList.filter((pokemon) =>
      bookmarkedIds.join(",").includes(pokemon.id)
    );
      ...
      <div className="bookmark-card-container">
        {pokemonData.length == 0 ? (
          <p className="no-bookmark-message">
            You haven't bookmarked any Pokémon yet!
          </p>
        ) : (
              ...
              >
                ...
                <div>
                  ...
                  {pokemon.locations ? (
                    ...
                    <p className="card-location">
                      <span className="card-title">Location</span>: Not avilable
                    </p>
                  )}
